...
try {
  ...
  await client.aclose();
        ...
        />
        <div className="rounded-lg border border-amber-400/20 bg-amber-400/5 p-4 text-sm text-ink-body">
          <strong className="text-ink-strong">Your deposit won't appear instantly.</strong>{' '}
          The 4Mica core only records a deposit after the block containing your transaction has
          been <em>finalized</em> on the underlying chain - not just confirmed by the sequencer.
          On Base this means waiting for the batch to be submitted to Ethereum L1 and reach
          Ethereum&apos;s finality, which typically takes <strong>10 to 30 minutes</strong>.
          Once finalized, your balance will update automatically and you can start making payments.
        </div>
      </div>
      <div className="space-y-4">
...
for (const pos of positions) {
  console.log("asset:", pos.asset, "collateral:", pos.collateral.toString());
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-semibold text-ink-strong">Load config from environment variables</h3>
        <p className="text-sm text-ink-body">
          Use <code className="font-mono">fromEnv()</code> to configure the client from environment
          variables - useful in CI, Docker, or any deployment where secrets are injected at runtime.
        </p>
        <CodeBlock
          language="bash"
          code={`# .env
...
const client = await Client.new(cfg);`}
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-semibold text-ink-strong">Coinbase CDP wallet (no private key)</h3>
        <p className="text-sm text-ink-body">
          Use a Coinbase Developer Platform MPC wallet - the private key never leaves CDP&apos;s HSM.
          Install the peer dependency, then pass the resulting account to{' '}
          <code className="font-mono">.signer()</code> instead of{' '}
          <code className="font-mono">.walletPrivateKey()</code>.
        </p>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-semibold text-ink-strong">Deposit USDC on Base Sepolia</h3>
        <p className="text-sm text-ink-body">
          For ERC-20 tokens (e.g. USDC), use <code className="font-mono">get_supported_tokens()</code> to
          resolve the token address from the network, then approve the Core4Mica contract and call{' '}
          <code className="font-mono">deposit</code>. The SDK resolves the contract address automatically
...
for pos in positions:
    print("asset:", pos.asset, "collateral:", pos.collateral)`}
      ...
      <div className="space-y-4">
        <h3 className="text-xl font-semibold text-ink-strong">Load config from environment variables</h3>
        <p className="text-sm text-ink-body">
          Use <code className="font-mono">from_env()</code> to configure the client from environment
          variables - useful in CI, Docker, or any deployment where secrets are injected at runtime.
        </p>
        <CodeBlock
          language="bash"
...
asyncio.run(main())`}
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-ink-body">
          Use a Coinbase Developer Platform MPC wallet - the private key never leaves CDP&apos;s HSM.
          Install the peer dependency, then pass the signer to{' '}
          <code className="font-mono">.signer()</code> instead of{' '}
